...
from extract_saliency_maps_v2 import (
    SaliencyCollector,
    fig_saliency_evolution,
    fig_saliency_overlay,
    fig_compute_heatmap,
    fig_cache_difficulty,
        ...
        if (self.previous_input[current_idx] is not None
            ...
            if hasattr(self, 'saliency_collector') and self.saliency_collector is not None:
                ...
                self.saliency_collector.record(
                    ...
                    gamma=None,
                    # grid_sizes from the forward pass — this is the key:
                    # WAN computes grid_sizes = (T_lat, H_after_patch, W_after_patch)
                    grid_sizes=grid_sizes[0] if grid_sizes is not None else None,
                    patch_size=getattr(self, 'patch_size', None),
                )
                if self.saliency_collector.step_count == 1:
                    print(f"[DEBUG] grid_sizes = {grid_sizes[0]}")
                    print(f"[DEBUG] saliency shape = {_sal_viz.shape}")
                    print(f"[DEBUG] expected SeqLen = {int(grid_sizes[0][0] * grid_sizes[0][1] * grid_sizes[0][2])}")
...
def generate(args):
    ...
    cls = wan_t2v.model.__class__
    cls.forward = worldcache_forward
    saliency_collector = SaliencyCollector()
    cls.saliency_collector = saliency_collector
    ...
    for prompt_file in tqdm(args.input_files, desc="Generating",
        ...
        if rank == 0 and saliency_collector.step_count > 0:
            viz_dir = os.path.join(args.output_dir, "saliency_viz")
            os.makedirs(viz_dir, exist_ok=True)
            name = file_data['name']

            # Saliency evolution + drift timeline
            fig_saliency_evolution(
                saliency_collector,
                save_path=os.path.join(viz_dir, f"{name}_evolution.pdf"),
                latent_frame=12)

            # Compute heatmap
            fig_compute_heatmap(
                saliency_collector,
                save_path=os.path.join(viz_dir, f"{name}_compute.pdf"),
                latent_frame=12)

            # Saliency overlay on video frames (if video available)
            if video is not None:
                frames_np = extract_video_frames(video, value_range=(-1, 1))
                fig_saliency_overlay(
                    saliency_collector, frames_np,
                    save_path=os.path.join(viz_dir, f"{name}_overlay.pdf"))

        # Reset collector for next prompt
        saliency_collector.reset()
